<script lang="ts">
  ...
  onMount(() => {
    ...
    dataSubscription = interval(duration).subscribe({
      next: (index) => {
        ...
        lineData = lineData.filter((data) => data.time >= new Date().getTime() - observingTime);
      },
    });
  });
  ...
  const chart = (selection: Selection<HTMLDivElement, RealtimeData[], HTMLDivElement, any>): void => {
    selection.each((data) => {
      ...
      const g = anotherSvg.select('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
